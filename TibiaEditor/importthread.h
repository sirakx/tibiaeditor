...
public:
	...
	void setIndex( qint32 index ) { m_startIndex = index; };
	void setItemList( const ItemList& items ) { m_itemList = items; };
	void setSpriteList( const ResourceList& resources ) { m_spriteList = resources; };
	void setFilter( bool filter ) { m_filter = filter; };
	void setResourceType( quint8 type ) { m_resourceType = type; };
	void setMode( ImportMode_t mode ) { m_mode = mode; };
	void setDestination( quint8 dest ) { m_destination = dest; };

	ImportMode_t getMode( void ) const { return m_mode; };
...
public:
	...
	virtual void setup( void );
	void setDropMode( DropMode_t dropMode ) { m_dropMode = dropMode; };
	void setUrlList( QUrlList& urlList );
