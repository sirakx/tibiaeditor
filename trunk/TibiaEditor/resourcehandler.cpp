...
SharedResource ResourceHandler::addResource( quint8 resourceType, quint32 identifier, quint16 frame, QFile* file )
{
	...
	SharedResource reference = SharedResource( new TibiaResource( resourceType, identifier, frame, file ) );
	foreignResources->push_back( reference );
	return reference;
}

SharedResource ResourceHandler::addResource( quint8 resourceType, SharedResource& resource )
{
	ResourceList * foreignResources;
	switch( resourceType )
	{
