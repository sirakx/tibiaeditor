...
public:
	...
	virtual bool idle( const QString&, bool ){ return false; };

	virtual ItemFile* getItemFile() { return this; };
	virtual const ItemFile* getItemFile() const { return this; };

	void setFormat( DatFormat* format ) { m_datFormat = format; };
	DatFormat* getFormat( void ) const { return m_datFormat; };
	...
	void setItemCount( quint16 items ) {				QMutex mutex;	mutex.lock();	m_items = items;				mutex.unlock();	};
	void setOutfitCount( quint16 outfits ) {			QMutex mutex;	mutex.lock();	m_outfits = outfits;			mutex.unlock();	};
	void setEffectCount( quint16 effects ) {			QMutex mutex;	mutex.lock();	m_effects = effects;			mutex.unlock();	};
	void setProjectileCount( quint16 projectiles ) {	QMutex mutex;	mutex.lock();	m_projectiles = projectiles;	mutex.unlock();	};

	quint32 getFirst() const { return m_begin; };
	quint32 getOutfitsBegin() const { return m_items+1; };
	quint32 getEffectsBegin() const { return m_items+m_outfits+1; };
	quint32 getProjectilesBegin() const { return m_items+m_outfits+m_effects+1; };
	quint32 getLast() const { return m_items+m_outfits+m_effects+m_projectiles; };

	quint16 getItemCount( void ) const { return m_items; };
	quint16 getOutfitCount( void ) const { return m_outfits; };
	quint16 getEffectCount( void ) const { return m_effects; };
	quint16 getProjectileCount( void ) const { return m_projectiles; };

	ItemList& getItems( void ) { return m_itemList; };
	ItemList& getOutfits( void ) { return m_outfitList; };
	ItemList& getEffects( void ) { return m_effectList; };
	ItemList& getProjectiles( void ) { return m_projectileList; };
	...
	UserThread* getThread( void ) const { return m_thread; };
